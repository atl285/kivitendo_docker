...
[ -n "${DB_PASSWORD}" ] && sed -i "1s/password = .*/password = ${DB_PASSWORD}/" /var/www/kivitendo-erp/config/kivitendo.conf
...
[ -n "${SMTP_PASSWORD}" ] && sed -i "2s/password =.*/password = ${SMTP_PASSWORD}/" /var/www/kivitendo-erp/config/kivitendo.conf
